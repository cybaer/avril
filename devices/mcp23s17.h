...
namespace avrlib
{
  ...
  public:
    ...
    {
      SlaveSelect::Low();
      SpiMasterBase::Send(slaveAddress | MCP23S17_WRITE);
    ...
    {
      SlaveSelect::Low();
      SpiMasterBase::Send(slaveAddress | MCP23S17_READ);
      SpiMasterBase::Send(address);
    ...
    static inline void ReadIO(void)
    {
      ...
      Ports[PORT_B].Gpio |= (Ports[PORT_B].IoDir & Read(MCP23S17_GPIOB));
    }
    ...
    static inline void Init(void)
    {
      ...
      /*Ports[PORT_A].Gpio = 0xff;
      Ports[PORT_B].Gpio = 0xff;
      Ports[PORT_A].IoDir = 0x00;
      Ports[PORT_B].IoDir = 0x00;
      */
  ...
  template<typename SpiMasterBase, typename SlaveSelect, uint8_t addr>
  Registers MCP23S17<SpiMasterBase, SlaveSelect, addr>::Ports[2];
